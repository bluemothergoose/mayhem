...
const minRange = 10; // The minimum ability range (doesn't make much sense to diagram a 5-feet-range ability unless it's an AOE), in feet
const maxRange = 300; // The maximum ability range (prevent drawing humongous grids for abilities with very long range), in feet
...
export default {
  ...
  props: {
    app: Object,
    range: Number, // The range of the ability, in feet
    aoe: Object, // A hash that represents the Area Of Effect of this ability,
  },
  data: () => { return {
  ...
  computed: {
    ...
    enabled() { // Whether this diagram should be drawn, or adds no value to the given ability and should be hidden
  ...
  updated() { // When the Diagram is updated (because the user clicked on another ability), re-draw the canvas
    this.draw();
  },
  methods: {
    draw() {
    ...
    positionTarget: function() {
      ...
      if (this.aoe) {
        if (this.aoe.type == 'cone') {
          ...
          this.target.x += cellSize / 100;
          this.target.y += cellSize / 100;
        }
      }
    },
        ...
        switch (this.aoe.type) {
          case "sphere":
            if (this.aoe.radius <= maxAOERadius) {
              ...
              ctx.stroke();
              ctx.fillStyle = "#ccc";
              ctx.fillText(this.aoe.radius + "-foot radius", target.x - 38, target.y - radiusInPixels - 4); // 38 and 4 are values to roughly center the text
          ...
          case "cube": {
            ...
            if (halfEdge <= maxAOERadius) {
              ...
              ctx.stroke();
              ctx.fillStyle = "#ccc";
              ctx.fillText(this.aoe.edge + "-foot cube", target.x - 38, target.y - halfEdge * pixelsPerFoot - 12);
              // Highlight individual squares
              ctx.setLineDash([]);
          ...
          case "cone":
            if (this.aoe.length <= maxAOERadius) {
              ...
              ctx.stroke();
              ctx.fillStyle = "#ccc";
              ctx.fillText(this.aoe.length + "-foot cone", origin.x - 45, origin.y - 40);
              // Highlight individual squares
              ctx.setLineDash([]);
    ...
    drawTarget(ctx) {
      ...
      if (this.range >= minRange) {
        ...
        ctx.lineTo(this.aoe ? target.x : target.x - circleRadius, target.y); // unless ability is AOE, end the line a bit earlier so that it doesn't overlap the mob circle
        ctx.setLineDash([4, 6]); // 5px long, 3px separated
        ctx.stroke();
    ...
    // Calculate the optimal amount of grid rows that should be used to represent this ability
    ...
    // Calculate the optimal amount of grid cols that should be used to represent this ability
