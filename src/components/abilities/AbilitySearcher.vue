  ...
  <div class="row no-gutters">
    <div class="col-12 col-lg-6 query-area pr-2">
      ...
      <div class="search-actions">
        ...
        <span class="d-none d-md-inline-block" @click="$emit('toggleDetailedView')">
          <img :src="require('@/assets/images/icons/misc/details.png')" :class="`details-icon ${detailedModeOn ? 'active' : ''}`" title="Toggle detailed ability list"/>
        </span>
    ...
    <div class="col-12 col-lg-6 filters">
      <div class="row no-gutters">
        ...
        <div class="col-4 filter">
          <CustomSelect v-model="query.category" :options="categoryOptions" align="right"/>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    levelOptions: function() {
      return [
        { value: '', text: 'Any level' }, { value: '0', text: 'Basic' }, { value: '1', text: '1st level' },
        { value: '2', text: '2nd level' }, { value: '3', text: '3rd level' }
      ].concat([4,5,6,7,8,9,10].map(n => { return { value: n.toString(), text: n + 'th level' }}));
    },
    categoryOptions: function() {
      return [{value: '', text: 'Any category', image: require('@/assets/images/icons/misc/none.png')}].
      concat(this.app.categories.map(s => { return { value: s, text: s, image: Icons.categoryIcon({category: s}) }}));
    }
  },
  methods: {
  ...
  watch: {
    rawQueryText: function() {
      ...
      debounce(() => {
        // If the query starts with a period, it means the user wants to search in the ability description too
